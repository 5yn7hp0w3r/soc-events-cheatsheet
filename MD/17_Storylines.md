...
> Типовые цепочки атак по MITRE ATT&CK / Cyber Kill Chain: что это за атака, сводка и пошаговая детализация с Event ID, ключевыми полями и логикой корреляции.
...
💡 **Что это за атака:** Атакующий перебирает пароли к одной учётке, пока не подберёт. В логах — вал неудачных входов (4625), затем успешный (4624). Часто по внешним сервисам (RDP, VPN, OWA).
...
💡 **Что это за атака:** Один-два популярных пароля пробуются против МНОГИХ учёток сразу — чтобы не словить блокировку по каждой. Признак: много 4625 по разным пользователям с одного источника.
...
💡 **Что это за атака:** Атакующий запрашивает Kerberos-билеты (TGS) для сервисных учёток с SPN и взламывает их офлайн, восстанавливая пароль сервисного аккаунта (часто с высокими правами). Признак — 4769 с шифрованием RC4.
...
💡 **Что это за атака:** Для учёток с отключённой преаутентификацией Kerberos можно получить зашифрованный ответ (AS-REP) и подобрать пароль офлайн. Признак: 4768 с Pre-Auth=0 и RC4.
...
💡 **Что это за атака:** Имея права репликации, атакующий выдаёт себя за контроллер домена и «сливает» хеши паролей (включая krbtgt) через штатный протокол репликации AD. Признак: 4662 с DS-Replication-Get-Changes от не-DC.
...
💡 **Что это за атака:** Классическое боковое перемещение через PsExec (Sysinternals): подключение к admin-шаре, установка службы и запуск команд на удалённом хосте. Признак: 5140 (ADMIN$) → 7045 → 4624 LT=3.
...
💡 **Что это за атака:** Удалённое выполнение команд через WMI — «тихий» способ бокового движения, часто без записи на диск. Признак: 4624 LT=3, затем 4688 с родителем WmiPrvSE.exe / процессом wmic.exe.
...
💡 **Что это за атака:** Перемещение между хостами по RDP с уже добытыми учётными данными. Признак: 4624 LT=10 → 1149 → 4778 внутри периметра.
...
💡 **Что это за атака:** Аутентификация по NTLM-хешу пароля без знания самого пароля: атакующий украл хеш из памяти и «прошёл» им. Признак: 4624 LT=3 по NTLM без обычного интерактивного входа.
...
💡 **Что это за атака:** Закрепление через планировщик задач — задача запускает payload по расписанию или триггеру, переживая перезагрузку. Признак: 4698 (создание задачи) → запуск payload.
...
💡 **Что это за атака:** Атакующий создаёт учётку или добавляет существующую в привилегированную группу (Domain Admins) для скрытого постоянного доступа. Признак: 4720 → 4728/4732.
...
💡 **Что это за атака:** Обход контроля учётных записей (UAC) для повышения до высокой целостности без запроса подтверждения — через автоэлевацию доверенных бинарей. Признак: 4688 с Token Elevation.
...
💡 **Что это за атака:** Заметание следов: очистка журнала Security и отключение аудита, чтобы скрыть активность. Признак: 1102 (журнал очищен) → 4719 (изменена политика аудита).
...
💡 **Что это за атака:** Перед шифрованием вымогатель удаляет теневые копии и отключает восстановление, затем массово меняет файлы. Признак: 4688 (vssadmin/wbadmin/bcdedit) → всплеск изменений файлов.
...
💡 **Что это за атака:** Аутентификация по украденному или поддельному сертификату через Kerberos PKINIT — получить билет без пароля. Признак: 4768 с заполненным полем Certificate Information.
...
💡 **Что это за атака:** Имея хеш служебной учётки krbtgt, атакующий сам подделывает «золотой» TGT с любыми правами и сроком жизни — это полный и долговременный контроль над доменом. Признак: 4769 без парного 4768.
...
💡 **Что это за атака:** Подделка сервисного билета (TGS) по хешу сервисной или машинной учётки. Даёт доступ к конкретному сервису БЕЗ обращения к контроллеру домена — поэтому очень тихо (на DC следов нет).
...
💡 **Что это за атака:** Типовой первичный доступ: пользователь открывает вложение, макрос запускает PowerShell, тот скачивает и запускает вредонос и устанавливает связь с C2-сервером.
...
💡 **Что это за атака:** Загрузка вредоноса штатными доверенными утилитами Windows (certutil, bitsadmin) — чтобы обойти антивирус «руками самой системы» (living-off-the-land).
...
💡 **Что это за атака:** Атакующий выгружает память процесса lsass, достаёт из неё пароли, хеши и билеты, и использует их для входа на другие хосты (боковое перемещение).
...
💡 **Что это за атака:** Закрепление и запуск payload через новую системную службу: стартует автоматически и от имени SYSTEM. Признак: 7045 с подозрительным путём (TEMP, cmd, powershell).
...
💡 **Что это за атака:** После эксплуатации веб-сервера (Exchange/IIS) атакующий кладёт веб-шелл и выполняет команды от имени процесса веб-сервера (w3wp.exe). Признак: создание .aspx в wwwroot + w3wp → cmd.
...
💡 **Что это за атака:** Современный захват учётки без сброса пароля: атакующий дописывает «теневые» ключи в атрибут msDS-KeyCredentialLink цели и логинится под ней по сертификату (PKINIT).
...
💡 **Что это за атака:** Атакующий принуждает контроллер домена к аутентификации (PetitPotam) и реле её на уязвимый веб-энролмент AD CS (ESC8); получает сертификат DC и через него — полный контроль над доменом.
