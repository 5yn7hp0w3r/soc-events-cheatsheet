...
| 🧷 **JSON** | [`json/`](json/) | **Источник правды.** Питает поиск на сайте; сюда вносим правки |

> ⚙️ **Важно для правок:** источник данных — папка [`json/`](json/). Файлы в [`MD/`](MD/) и `data.js` — это **автоматические/статичные копии**, их править не нужно (они не влияют на источник). Как дополнять — в [CONTRIBUTING.md](CONTRIBUTING.md).
...
Проект развивается силами комьюнити, и дополнять его **просто** — git знать не нужно, всё правится прямо в браузере. Полная инструкция: **[CONTRIBUTING.md](CONTRIBUTING.md)**.

- ✏️ **Поправить/добавить** — откройте нужный файл в [`json/`](json/), нажмите карандаш, внесите правку → Pull Request. Робот проверит JSON на ошибки, после принятия сайт обновится сам.
- 🐛 **Не хотите возиться с JSON?** Откройте **Issue** по форме «Предложить или исправить» — оформим сами.
- ⭐ Поставьте звезду, если справочник оказался полезен.

> 🛡️ Данные проверяются автоматически (JSON-schema в CI) — сломать справочник через PR нельзя.
